...
export class BooksParentListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    {
    this.books=obs.filter((obsItem:any)=>obsItem.isBookActive==1);}
    )
      if(this.user.role==false)
      this.store.getIssuesByUserId(this.user.id).subscribe(
        (issues:any)=>{
      ...
      this.store.issuesSubject.subscribe((issues:any)=>{
        this.adminActiveIssues=issues.filter((issue:any)=>issue.isActive==1);
        this.adminPastIssues=issues.filter((issue:any)=>issue.isActive==0);
      })}
    }
